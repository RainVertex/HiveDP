...
export function UserAutocomplete({ value, onChange, placeholder, onSelect, className }: Props) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const t = setTimeout(() => {
      fetch(`/api/vikunja/users/search?q=${encodeURIComponent(q)}`)
        .then((r) => (r.ok ? r.json() : []))
        .then((d: VikunjaUser[]) => {
          if (!cancelled) {
            setSuggestions(d ?? []);
  ...
  function handlePick(u: VikunjaUser) {
    onChange(u.username);
