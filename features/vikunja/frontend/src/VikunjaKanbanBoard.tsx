...
interface BoardBucket extends VikunjaBucket {
  platform_id?: string;
  tasks: VikunjaTask[];
}
...
export function VikunjaKanbanBoard({ projectId, tasks, onUpdate, canEdit = true }: Props) {
  ...
  const [activeTask, setActiveTask] = useState<VikunjaTask | null>(null);
  const [newBucketTitle, setNewBucketTitle] = useState("");
  const [showNewBucket, setShowNewBucket] = useState(false);
  const [editingBucket, setEditingBucket] = useState<number | null>(null);
  const [editingTitle, setEditingTitle] = useState("");
  const [newTaskBucket, setNewTaskBucket] = useState<number | null>(null);
  const [newTaskTitle, setNewTaskTitle] = useState("");

  const sensors = useSensors(useSensor(PointerSensor, { activationConstraint: { distance: 5 } }));

  useEffect(() => {
    const apiBucketsTyped = apiBuckets as Array<VikunjaBucket & { platform_id?: string }>;
    if (apiBucketsTyped.length === 0 && tasks.length > 0) {
      setBuckets([{ id: 0, title: "All Tasks", position: 0, tasks }]);
      return;
    }
    const tasksByBucket = new Map<number, VikunjaTask[]>();
    for (const t of tasks) {
      const list = tasksByBucket.get(t.bucket_id) ?? [];
      list.push(t);
      tasksByBucket.set(t.bucket_id, list);
    }
    setBuckets(
      apiBucketsTyped
        .slice()
        .sort((a, b) => a.position - b.position)
        .map((b) => ({ ...b, tasks: tasksByBucket.get(b.id) ?? [] })),
    );
  }, [apiBuckets, tasks]);
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    const droppableMatch = /^bucket-(\d+)$/.exec(overId);
    if (droppableMatch) {
      targetBucketId = Number(droppableMatch[1]);
    } else {
      for (const b of buckets) {
        if (String(b.id) === overId) {
          targetBucketId = b.id;
          break;
        }
        if (b.tasks.some((t) => String(t.id) === overId)) {
          targetBucketId = b.id;
          break;
        }
      }
    ...
    if (!task || task.bucket_id === targetBucketId) return;

    try {
      await update(taskId, { bucket_id: targetBucketId });
      onUpdate();
      refetchBuckets();
    } catch {
  ...
  async function commitRename(bucket: BoardBucket) {
    ...
    if (!bucket.platform_id) {
      setEditingBucket(null);
      return;
    }
    try {
      await updateBucket(bucket.platform_id, { title: editingTitle.trim() });
      refetchBuckets();
    } catch {
      // ignore
    } finally {
  ...
  async function handleAddTask(bucketId: number, e: React.FormEvent) {
    e.preventDefault();
    if (!newTaskTitle.trim()) return;
    try {
      await createTask({ title: newTaskTitle.trim(), bucket_id: bucketId });
      setNewTaskTitle("");
            ...
            <div className="flex items-center justify-between border-b border-app-border px-3 py-2">
              {editingBucket === bucket.id ? (
                ...
                <h3
                  ...
                  onDoubleClick={() => canEdit && bucket.platform_id && startRename(bucket)}
